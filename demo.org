...
- Simple tooling to run generation.
...
~cogs~ to be used.  It then gives details on the code generation steps
and ends with a section that uses the demo's schema to illustrate how
one may write their own.
